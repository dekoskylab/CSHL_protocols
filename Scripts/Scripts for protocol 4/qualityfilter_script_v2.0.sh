	...
	awk '/>/{temp=$0; getline; print temp "\t" $0}' $file1"_q20p50.fasta" | sed -r 's/1:N:0:[NATGC]{6}.[NATGC]{6}/_F/' | sort -k 1b,1  > $file1"_q20p50.fasta_temp"
	awk '/>/{temp=$0; getline; print temp "\t" $0}' $file2"_q20p50.fasta" | sed -r 's/2:N:0:[NATGC]{6}.[NATGC]{6}/_R/' | sort -k 1b,1  > $file2"_q20p50.fasta_temp"
	...
	split -l 100000 $prefix"_r1_q20p50_filtered.fasta" $prefix"_r1_q20p50_filtered.fasta_"
	split -l 100000 $prefix"_r2_q20p50_filtered.fasta" $prefix"_r2_q20p50_filtered.fasta_"
	
		
	ls | grep $prefix"_r2_q20p50_filtered.fasta_" | grep -v temp | grep -v reads | awk '{print $1 " " $1}' | sed 's/_r2_q20p50/_r1_q20p50/' | awk -v barcodes=$barcodes '{print "bash CDR3motif_search_part1_v3.0.sh " $0 " " barcodes}' > $prefix"_commands_part1"
